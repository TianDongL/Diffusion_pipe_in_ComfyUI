...
class QwenImageModelNode:
        ...
        return {
            "required":{
                "model_version": (["qwen_image", "qwen_edit_2511", "qwen2512", "qwen_edit_2509", "qwen_edit"], {
                    "default": "qwen_image",
                    "tooltip": "选择Qwen模型版本：qwen_image、qwen_edit、qwen_edit_2511、qwen2512或qwen_edit_2509"
                })
            },
            "optional": {
                "diffusers_path": ("STRING", {
    ...
    def get_qwen_image_config(self, model_version: str, transformer_path: str = "", text_encoder_path: str = "", 
        ...
        try:
            if model_version == "qwen_edit_2511":
                model_type = "qwen2511"
            else: 
    ...
    def get_aura_flow_config(self, transformer_path: str, text_encoder_path: str = "", vae_path: str = "", max_sequence_length: int = 768) -> Tuple[dict]:
        try:
            ...
            config = {
                "type": "auraflow",
                "transformer_path": normalized_transformer_path,
                "text_encoder_path": normalize_wsl_path(text_encoder_path.strip()),
                "vae_path": normalize_wsl_path(vae_path.strip()),
...
class hunyuanvideo15ModelNode:
    ...
    def get_hunyuan_video_15_config(self, diffusion_path: str, vae_path: str, Text_Encoder: str, ByT5: str, shift: int) -> Tuple[dict]:
        try:
            ...
            if diffusion_path.strip():
                config["diffusion_model"] = normalize_wsl_path(diffusion_path.strip())
            else:
                return ({"error": "Diffusion路径不能为空"},)
            
            if vae_path.strip():
                config["vae"] = normalize_wsl_path(vae_path.strip())
            else:
                return ({"error": "VAE路径不能为空"},)
            ...
            config["text_encoders"] = [
                {
                    "paths": [
                        normalize_wsl_path(Text_Encoder.strip()),
                        normalize_wsl_path(ByT5.strip())
                    ],
                    "type": "hunyuan_video_15"
                }
            ]            
